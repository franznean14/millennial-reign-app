...
DO $$ BEGIN
  CREATE TYPE public.business_establishment_status_t AS ENUM (
    'for_scouting','for_follow_up','accepted_rack','declined_rack','has_bible_studies','inappropriate','on_hold'
  );
EXCEPTION
  WHEN duplicate_object THEN null;
END $$;
...
DO $$ BEGIN
  ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS is_congregation_guest boolean NOT NULL DEFAULT false;
EXCEPTION
  WHEN duplicate_column THEN null;
END $$;
...
-- Guest name inheritance (optional link when adding a publisher; 1 guest name <-> 1 profile per congregation)
...
CREATE TABLE IF NOT EXISTS public.event_schedules (
  ...
  -- Constraints
  CONSTRAINT event_schedules_ministry_type_check CHECK (
    (event_type = 'ministry' AND ministry_type IS NOT NULL) OR
    (event_type != 'ministry' AND ministry_type IS NULL)
  ),
  CONSTRAINT event_schedules_date_range_check CHECK (end_date IS NULL OR end_date >= start_date),
  CONSTRAINT event_schedules_time_range_check CHECK (
    start_time IS NULL OR 
    end_time IS NULL OR 
    end_time >= start_time OR
    is_all_day = true
  ),
  CONSTRAINT event_schedules_recurrence_end_check CHECK (recurrence_end_date IS NULL OR recurrence_end_date >= start_date)
);
...
-- Profiles: restrict self-service updates (RLS allows any user to update own row; trigger enforces bounds)
...
-- Event schedules updated_at trigger
...
-- Read profiles referenced as publisher/partner on congregation-scoped calls / todos (incl. after removal from congregation).
...
-- Unclaimed guest names from visits/todos (elders of congregation, admin_users, or profile admin/superadmin)
...
AS $$
  WITH allowed AS (
    SELECT (
      ...
      OR EXISTS (
        ...
        WHERE me.id = auth.uid()
          AND (
            (me.privileges @> array['Elder']::text[] AND me.congregation_id = congregation_id_param)
            OR me.role IN ('admin', 'superadmin')
          )
      )
    ) AS ok
  ...
  raw_names AS (
    ...
    WHERE t.publisher_id IS NULL
      ...
      AND COALESCE(t.congregation_id, c.congregation_id) = congregation_id_param
    UNION
...
BEGIN
  ...
  WHERE t.publisher_id IS NULL
    ...
    AND COALESCE(
      t.congregation_id,
      (SELECT c2.congregation_id FROM public.calls c2 WHERE c2.id = t.call_id LIMIT 1)
    ) = congregation_id_param;
...
-- User search function for elders (can find users from any congregation or no congregation)
...
RETURNS TABLE (
  ...
  -- Check if current user is an elder, then search all users
  SELECT 
    p.id as user_id,
    p.first_name,
    p.last_name,
  ...
  WHERE (p.username = search_term OR u.email = search_term)
    ...
    -- Remove congregation restriction - elders can search users from any congregation
    -- This allows finding users without congregation or from different congregations
$$;
