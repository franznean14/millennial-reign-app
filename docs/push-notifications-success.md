...
### **VAPID Keys (Working)**
```bash
# Generated by web-push library (Updated for Production)
NEXT_PUBLIC_VAPID_PUBLIC_KEY=BEOEIjVPVTcmYF5zmodxTOjJ24R3mR-l7WOi2X1zcEnnoGH6NTtmpiQk8GRAIyofu5zdJIY3BEtu3n4igFD-yhw
VAPID_PRIVATE_KEY=0XaiN82JbTfqKgv45sU4twHTHcWNP-VNvzTuFKS8Ujk
```
...
1. **Add VAPID keys** to your production environment variables
2. **Clear existing subscriptions** (users will need to re-enable)
3. **Redeploy** your application
4. **Test notifications** on production domain

### **Clear Production Subscriptions**
```bash
# Run this script after updating VAPID keys
SUPABASE_URL=your_production_url SUPABASE_SERVICE_ROLE_KEY=your_service_key node scripts/clear-production-subscriptions.js
```
