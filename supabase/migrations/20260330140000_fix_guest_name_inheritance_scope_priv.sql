-- Fix guest name list/inherit: (1) scope call_todos by COALESCE(t.congregation_id, call.congregation_id)
-- so rows backfilled only on the call are not dropped; (2) allow profiles with role admin/superadmin
-- to match app congregation management (not only admin_users + elders).
