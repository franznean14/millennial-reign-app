...
async function checkServiceWorker(): Promise<DiagCheck> {
  ...
  if (!("serviceWorker" in navigator)) {
    return { id, label, status: "fail", details: "serviceWorker API unavailable" };
  }
  try {
...
export default function DiagnosticsPage() {
  ...
  return (
    <main className="min-h-screen bg-background p-4 pb-[calc(max(env(safe-area-inset-bottom),0px)+80px)]">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">Use this screen when a user reports app load failures.</span>
            </div>
