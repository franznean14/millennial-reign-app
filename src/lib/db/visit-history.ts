...
type VisitQueryResult = {
  ...
  business_establishments?: { name?: string | null; statuses?: string[] | null } | null;
  householders?: { name?: string | null; establishment_id?: string | null } | null;
...
export async function getBwiVisitsPage({
  ...
  const cacheKey = `bwi-all-visits-${userId ?? "all"}-${offset}`;
  const cached = await cacheGet<{ visits?: VisitRecord[] }>(cacheKey);
